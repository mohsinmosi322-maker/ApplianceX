...
{
    public class AdminPurchaseModifyForm : Form
    {
        ...
        public AdminPurchaseModifyForm(string connectionString)
        {
            ...
            dgv.SelectionChanged += (s, e) =>
            {
                ...
                selectedId = Convert.ToInt32(dgv.CurrentRow.Cells["PurchaseID"].Value);
            ...
            btn.Click += (s, e) =>
            {
                ...
                {
                    c.Open();
                    using (var cmd = new SqlCommand("UPDATE PurchaseHeader SET PaidAmount=@P, BalanceAmount=@B, Remarks=@R WHERE PurchaseID=@ID", c))
                    {
        ...
        {
            using (var c = new SqlConnection(connStr))
            using (var da = new SqlDataAdapter("SELECT TOP 200 PurchaseID,InvoiceNo,PurchaseDate,TotalAmount,Discount,NetAmount,PaidAmount,BalanceAmount,Remarks FROM PurchaseHeader ORDER BY PurchaseDate DESC", c))
            {
