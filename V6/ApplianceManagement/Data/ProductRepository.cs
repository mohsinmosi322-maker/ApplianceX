...
namespace ApplianceManagement.Data
{
    public class ProductRepository
    {
        ...
        public List<Product> Search(string keyword)
        {
            ...
            {
                conn.Open();
                using (var cmd = DbHelper.CreateCommand(Base()+" WHERE p.IsActive=1 AND (p.ProductCode LIKE @kw OR p.Barcode LIKE @kw OR p.ProductName LIKE @kw) ORDER BY p.ProductName", conn))
                {
        ...
        public List<Product> GetLowStock()
        {
            ...
            {
                conn.Open();
                string sql = Base()+@" WHERE p.IsActive=1 AND ISNULL((SELECT SUM(QuantityIn)-SUM(QuantityOut) FROM InventoryTransaction WHERE ProductID=p.ProductID),0) <= p.MinimumStock
        ...
        public string GetNextProductCode()
        {
            ...
            {
                conn.Open();
                using (var cmd = DbHelper.CreateCommand("SELECT ISNULL(MAX(TRY_CAST(ProductCode AS INT)),0)+1 FROM Products WHERE ISNUMERIC(ProductCode)=1", conn))
        ...
        public int Insert(Product p)
        {
            ...
            {
                conn.Open();
                using (var cmd = DbHelper.CreateCommand("INSERT INTO Products(ProductCode,Barcode,ProductName,CategoryID,PurchasePrice,SalePrice,MinimumStock,IsActive) VALUES(@C,@B,@N,@Cat,@Pur,@Sale,@Min,1); SELECT SCOPE_IDENTITY();", conn))
                {
        ...
        public bool ExistsCode(string code)
        {
            using (var conn = DbHelper.GetConnection())
            {
                ...
                { cmd.Parameters.AddWithValue("@C", code); return Convert.ToInt32(cmd.ExecuteScalar())>0; }
            }
        }
        public bool ExistsBarcode(string barcode)
        {
            ...
            using (var conn = DbHelper.GetConnection())
            {
                ...
                { cmd.Parameters.AddWithValue("@B", barcode); return Convert.ToInt32(cmd.ExecuteScalar())>0; }
            }
        }
        ...
        private string Base() => @"SELECT p.*,c.CategoryName,ISNULL((SELECT SUM(QuantityIn)-SUM(QuantityOut) FROM InventoryTransaction WHERE ProductID=p.ProductID),0) AS CurrentStock FROM Products p LEFT JOIN Categories c ON p.CategoryID=c.CategoryID";
        private Product Map(SqlDataReader r) => new Product {
            ProductID=(int)r["ProductID"], ProductCode=r["ProductCode"].ToString(),
            ...
            PurchasePrice=(decimal)r["PurchasePrice"], SalePrice=(decimal)r["SalePrice"],
            MinimumStock=r["MinimumStock"]==DBNull.Value?0:(int)r["MinimumStock"],
            CurrentStock=Convert.ToInt32(r["CurrentStock"]), IsActive=(bool)r["IsActive"],
        ...
        public List<Product> SearchAll(string keyword)
        {
            ...
            {
                conn.Open();
                using (var cmd = DbHelper.CreateCommand(Base()+" WHERE (p.ProductCode LIKE @kw OR p.Barcode LIKE @kw OR p.ProductName LIKE @kw) ORDER BY p.ProductName", conn))
                {
                    cmd.Parameters.AddWithValue("@kw", "%"+keyword+"%");
                    using (var r = cmd.ExecuteReader()) while (r.Read()) list.Add(Map(r));
                }
        ...
        public void Update(int id, string name, decimal pur, decimal sale, int minStock, bool active)
        {
            ...
            {
                conn.Open();
                using (var cmd = DbHelper.CreateCommand("UPDATE Products SET ProductName=@N, PurchasePrice=@P, SalePrice=@S, MinimumStock=@M, IsActive=@A WHERE ProductID=@ID", conn))
                {
