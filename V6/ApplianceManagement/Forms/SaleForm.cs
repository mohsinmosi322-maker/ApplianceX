...
namespace ApplianceManagement.Forms
{
    public class SaleForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            {
                Text = "Type name/phone · Enter select · Walk-in if empty",
            ...
            var entry = new Panel { Dock = DockStyle.Top, Height = 56, BackColor = Color.White, Padding = new Padding(10, 6, 10, 6) };
            ...
            {
                Text = "\u2191\u2193 suggestions · Enter add · F8 remove · F9 history · F12 discount",
                Font = UiHelper.SmallFont,
                ForeColor = Color.Gray,
        ...
        private void Form_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Up && !lstProduct.Visible && !lstCustomer.Visible) { e.Handled = true; MoveGrid(-1); }
            if (e.KeyCode == Keys.Down && !lstProduct.Visible && !lstCustomer.Visible) { e.Handled = true; MoveGrid(1); }
        }
        ...
        private void ShowProductSuggestions()
        {
            ...
            {
                lstProduct.Items.Add(p.ProductCode + " \u2014 " + p.ProductName + "  |  Stock: " + p.CurrentStock.ToString("0.##"));
                map.Add(p);
            }
            lstProduct.Tag = map;
        ...
        {
            if (cart.Count == 0 || dgv.Rows.Count == 0) return;
            int idx = dgv.CurrentRow != null ? dgv.CurrentRow.Index : 0;
            idx += delta;
            if (idx < 0) idx = 0;
            if (idx >= dgv.Rows.Count) idx = dgv.Rows.Count - 1;
            dgv.ClearSelection();
            dgv.Rows[idx].Selected = true;
            try { dgv.CurrentCell = dgv.Rows[idx].Cells[0]; } catch { }
        }
