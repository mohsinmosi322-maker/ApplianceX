...
namespace ApplianceManagement.Forms
{
    public class SaleReturnForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            lblCustomer = new Label { Text = "Customer: —", Location = new Point(410, 12), AutoSize = true, Font = UiHelper.NormalFont, ForeColor = UiHelper.ThemeDark };
            ...
            if (invoiceLines != null && invoiceLines.Count > 0)
            {
                foreach (var line in invoiceLines)
                {
                    if ((line.ProductCode != null && line.ProductCode.IndexOf(q, StringComparison.OrdinalIgnoreCase) >= 0) ||
                        ...
                        list.Add(new Product { ProductID = line.ProductID, ProductCode = line.ProductCode, ProductName = line.ProductName, SalePrice = line.SalePrice });
                }
            }
            if (list.Count == 0)
            {
        ...
        private void Search_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (invoiceLines != null && invoiceLines.Count > 0)
            {
                ...
                if (line != null)
                {
                    selectedProduct = _products.GetById(line.ProductID) ?? new Product { ProductID = line.ProductID, ProductCode = line.ProductCode, ProductName = line.ProductName, SalePrice = line.SalePrice };
        ...
        private void AddLine()
        {
            ...
            int? origDetail = null; int sold = 0, already = 0; decimal rate = selectedProduct.SalePrice;
            if (invoiceLines != null)
            {
                var line = invoiceLines.FirstOrDefault(x => x.ProductID == selectedProduct.ProductID);
                if (line != null)
                {
                    origDetail = line.SaleDetailID; sold = line.SoldQty; already = line.AlreadyReturned; rate = line.SalePrice;
                    int returnable = Math.Max(0, sold - already);
                    int inCart = cart.Where(c => c.ProductID == selectedProduct.ProductID).Sum(c => c.Quantity);
                    if (qty + inCart > returnable) { DialogHelpers.Warn(this, "Returnable qty is " + returnable); return; }
                }
            }
            var existing = cart.FirstOrDefault(c => c.ProductID == selectedProduct.ProductID && c.OriginalSaleDetailID == origDetail);
            if (existing != null) { existing.Quantity += qty; existing.Amount = Math.Round(existing.Quantity * existing.SalePrice, 2); }
            else cart.Add(new SaleReturnDetail { OriginalSaleDetailID = origDetail, ProductID = selectedProduct.ProductID, ProductCode = selectedProduct.ProductCode, ProductName = selectedProduct.ProductName, Quantity = qty, SalePrice = rate, Amount = Math.Round(qty * rate, 2), SoldQty = sold, AlreadyReturned = already });
        ...
        private void Save()
        {
            if (cart.Count == 0) { DialogHelpers.Warn(this, "Add at least one product to return."); return; }
            if (string.IsNullOrWhiteSpace(txtReason.Text)) { DialogHelpers.Warn(this, "Return reason is required."); txtReason.Focus(); return; }
            if (!DialogHelpers.Confirm(this, "Save sale return? Stock will increase.")) return;
            decimal disc = 0, refund = 0; decimal.TryParse(txtDiscount.Text, out disc); decimal.TryParse(txtRefund.Text, out refund);
            var ret = new SaleReturnHeader { OriginalSaleID = originalSaleId, OriginalInvoiceNo = (txtInvoice.Text ?? "").Trim(), CustomerID = customerId > 0 ? customerId : 1, CustomerName = customerName, Discount = disc, RefundAmount = refund, Remarks = txtReason.Text.Trim(), ReturnDate = DateTime.Now, Details = cart.ToList() };
            try
            {
                _svc.Save(ret);
                DialogHelpers.Info(this, "Sale return saved.\nReturn No: " + ret.ReturnNo);
                cart.Clear(); invoiceLines.Clear(); originalSaleId = 0; customerId = 0; RefreshGrid();
                txtReason.Clear(); txtInvoice.Clear(); lblCustomer.Text = "Customer: —"; txtSearch.Focus();
            }
            catch (Exception ex) { DialogHelpers.Error(this, ex.Message); }
        }
    }
